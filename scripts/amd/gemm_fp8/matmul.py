...
def matmul(a, b, activation=""):
    ...
    USE_FP8 = True if b.dtype == torch.int8 else False
...
def run_speed(M, N, K, datatype, use_rocprof, provider):
    a = torch.randn((M, K), device='cuda', dtype=datatype)
    b = torch.randn((K, N), device='cuda', dtype=datatype)
    #b = (b*128).to(torch.int8)
    #b = torch.randn((K, N), device='cuda', dtype=torch.int8)
    b8 = f16_to_f8(b, dtypes = tl.float8e5)
    ...
    if provider == 'triton':
        ...
        ms, min_ms, max_ms = triton.testing.do_bench(lambda: matmul(a, b8), quantiles=quantiles)
    return min_ms
