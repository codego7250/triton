...
def tune_gemm(SIZE_M, SIZE_N, SIZE_K):
    ...
    golden = torch.matmul(a, b)
    """
    tval = triton_matmul(a,b,c,32,128,32,2,4)
    tval = triton_matmul(a,b,c,32,128,32,2,4)
    tval = triton_matmul(a,b,c,32,128,32,2,4)
    return

    if SIZE_N == 1040 and SIZE_K == 4608 :
        print("ttt")
        tval = triton_matmul(a,b,c,32,128,64,8,4)
        tval = triton_matmul(a,b,c,32,128,64,8,4)
        tval = triton_matmul(a,b,c,32,128,64,8,4)
    elif SIZE_N == 4608 and SIZE_K == 320 :
        tval = triton_matmul(a,b,c,32,64,32,10,2)
        tval = triton_matmul(a,b,c,32,64,32,10,2)
        tval = triton_matmul(a,b,c,32,64,32,10,2)
    elif SIZE_N == 16 and SIZE_K == 4608 :
        tval = triton_matmul(a,b,c,32,32,64,8,1)
        tval = triton_matmul(a,b,c,32,32,64,8,1)
        tval = triton_matmul(a,b,c,32,32,64,8,1)
    ...
    #1,16,12288
    exec_time = triton.testing.do_bench(lambda: triton_matmul(a, b, c, 32, 32, 128, 8, 2))
    #1,4992,12288
    #exec_time = triton.testing.do_bench(lambda: triton_matmul(a, b, c, 32, 64, 32, 4, 2))
    #1,12288,1536
    #exec_time = triton.testing.do_bench(lambda: triton_matmul(a, b, c, 32, 128, 32, 2, 4))
