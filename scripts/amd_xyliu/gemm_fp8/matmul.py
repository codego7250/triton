...
def tune_gemm(SIZE_M, SIZE_N, SIZE_K):
    ...
    b8 = f16_to_f8(b, dtypes = tl.float8e5)
    ...
    tvalue = triton_matmul(a,b,c,32,64,128,1,2)
    tvalue = triton_matmul(a,b,c,32,64,128,1,2)
    tvalue = triton_matmul(a,b,c,32,64,128,1,2)

    if torch.allclose(c.half(), golden, atol=1e-2, rtol=0):
    ...
    exec_time = triton.testing.do_bench(lambda: triton_matmul(a, b8, c, 32, 128, 64, 5, 4))
    print(f"exec_time 8weight = {exec_time} ms", )
    ...
    print(f"exec_time 16 = {exec_time} ms", )
    return
